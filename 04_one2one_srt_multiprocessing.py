    ...
    for run in range(1,101):
        ...
        for i in listOfIsSample:
            if os.path.isfile(srcFolder+i):
                ...
                os.rename(srcFolder+i+".temp", srcFolder+i+".processed") # rename into processed
...
if __name__ == '__main__':
    jobs = []
    for i in range(12):
        p = multiprocessing.Process(target=one2one, args=(folder,i+1,))
        jobs.append(p)
        p.start()
